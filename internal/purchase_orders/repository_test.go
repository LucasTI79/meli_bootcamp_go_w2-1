...
func TestRepositoryGet(t *testing.T) {
	t.Run("Should return a purchase order by specified id", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta(purchase_orders.GetQuery)).
			WithArgs(purchaseOrderID).
			WillReturnRows(rows)

		repository := purchase_orders.NewRepository(db)
...
func TestRepositoryExists(t *testing.T) {
	t.Run("Should return true", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta(purchase_orders.ExistsQuery)).
	...
	t.Run("Should return false when there are no query results", func(t *testing.T) {
		...
		mock.ExpectQuery(purchase_orders.ExistsQuery).WithArgs(orderNumber).WillReturnError(sql.ErrNoRows)
	...
	t.Run("Should return false when database has internal error", func(t *testing.T) {
		...
		mock.ExpectQuery(purchase_orders.ExistsQuery).WithArgs(orderNumber).WillReturnError(sql.ErrConnDone)
...
func TestRepositorySave(t *testing.T) {
	t.Run("Should insert the purchase order and return the purchase order id", func(t *testing.T) {
		...
		mock.ExpectPrepare(regexp.QuoteMeta(purchase_orders.InsertQuery))
		mock.ExpectExec(regexp.QuoteMeta(purchase_orders.InsertQuery)).
	...
	t.Run("Should throw panic when expected prepare fails", func(t *testing.T) {
		...
		mockedPurchaseOrder := mockedPurchaseOrderTemplate
		mock.ExpectPrepare(regexp.QuoteMeta(purchase_orders.InsertQuery)).WillReturnError(sql.ErrConnDone)
